...
def main():
	...
	parser.add_argument("--input_file", "-i", required = True, type = input_check, nargs = "+",
			...
			If you want to create a dataset for training, you neet to add an third tab \
			for experimental binding affinity for the complex.")	
...
def preprocessing(lines):
	...
	data = list()
	for line in lines[1:]:
		data.append([os.path.abspath(i.strip()) for i in line.split("\t")]) 
