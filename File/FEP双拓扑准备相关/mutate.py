...
''', formatter_class=argparse.RawTextHelpFormatter)
    ...
    exclus = parser.add_mutually_exclusive_group()   # 互斥组，只能执行组内选项的其中一个。
    ...
    parser.add_argument('-f',
                        ...
                        default='protein.pdb')   # 输入的pdb或者gro格式文件名字
    ...
    parser.add_argument('--script',
                        ...
                        default=None)   # 将突变的信息写成文件的形式，如果不提供文件也可以交互选择。
    exclus.add_argument('--keep_resid',
                        ...
                        default=True,
                        action='store_false')   # 保留输入文件的残基编号信息，否则会将残基全部进行重新编号。
    exclus.add_argument('--ref',
                        ...
                        default=None)   # 和--keep_resid不能同时使用。如果对输入进行了重新编号，可以同时提供一个没有重新编号的参考文件，然后根据参考文件的残基编号进行选择突变。
    parser.add_argument('--resinfo',
                        dest='resinfo',
                        help='Show the list of 3-letter -> 1-letter residues',
                        default=False,
                        action='store_true')   # 列出残基的 3字母 和 1字母 的对应信息。
